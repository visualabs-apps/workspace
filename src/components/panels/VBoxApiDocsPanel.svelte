    ...
    <div class="border-b border-gray-200 px-5 py-3 bg-white">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-sm font-bold text-gray-900">VBox API Reference</h2>
                    <p class="text-xs text-gray-500">{totalApis} APIs across {apiSections.length} categories</p>
                </div>
            </div>
            <div class="flex items-center gap-1">
                <button
                    onclick={expandAll}
                    class="p-1.5 hover:bg-gray-100 rounded transition-colors text-gray-500"
                    title="Expand all"
                >
                    <Expand size={15} />
                </button>
                ...
                <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded transition-colors {copiedMarkdown ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200'}"
    ...
    <div class="flex-1 overflow-y-auto">
        ...
        <div class="border-b border-gray-200">
            <button
                ...
                class="w-full flex items-center gap-2 px-5 py-3 bg-gray-800 hover:bg-gray-700 transition-colors"
            >
            ...
            {#if expandedSections.has('section-quickstart')}
                <div class="px-5 py-3 bg-gray-50">
                    <pre class="bg-gray-900 text-gray-100 p-4 rounded text-xs overflow-x-auto leading-relaxed"><code>{quickStartCode}</code></pre>
                </div>
            {/if}
        </div>
        ...
        {#each apiSections as section, sIdx}
            ...
            <div class="border-b border-gray-200">
                ...
                <button
                    ...
                    style="background-color: {section.color}15; border-left: 3px solid {section.color};"
                >
                    ...
                    <span class="text-xs text-gray-500 ml-auto">{section.apis.length} endpoint{section.apis.length > 1 ? 's' : ''}</span>
                </button>
                        ...
                        {#each section.apis as api, aIdx}
                            ...
                            <div class="border-t border-gray-100">
                                <button
                                    ...
                                    class="w-full flex items-center gap-3 px-5 py-2.5 hover:bg-gray-50 transition-colors text-left"
                                >
                                    <!-- Method Badge -->
                                    <span
                                    ...
                                    <!-- Path / Name -->
                                    <code class="text-sm font-medium text-gray-900 truncate">{api.name}</code>
                                    <!-- Description -->
                                    <span class="text-xs text-gray-500 truncate hidden md:inline ml-1">{api.description}</span>
                                    <!-- Expand indicator -->
                                    {#if isApiExpanded}
                                        <ChevronDown size={14} class="text-gray-400 ml-auto shrink-0" />
                                    {:else}
                                        <ChevronRight size={14} class="text-gray-400 ml-auto shrink-0" />
                                    {/if}
                                </button>

                                <!-- Expanded Detail (Swagger operation detail) -->
                                {#if isApiExpanded}
                                    <div class="px-5 pb-4 pt-1 bg-white border-t border-gray-100">
                                        <div class="pl-16 space-y-3">
                                            <!-- Description -->
                                            <p class="text-sm text-gray-700">{api.description}</p>

                                            <!-- Parameters -->
                                            {#if api.params && api.params.length > 0}
                                                <div>
                                                    <h4 class="text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Parameters</h4>
                                                    <table class="w-full text-xs border border-gray-200 rounded overflow-hidden">
                                                        <thead>
                                                            <tr class="bg-gray-50">
                                                                <th class="text-left px-3 py-2 font-semibold text-gray-600 border-b border-gray-200">Name</th>
                                                                <th class="text-left px-3 py-2 font-semibold text-gray-600 border-b border-gray-200">Type</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {#each api.params as param}
                                                                <tr class="border-b border-gray-100 last:border-0">
                                                                    <td class="px-3 py-1.5 font-mono text-blue-700">{param.name}</td>
                                                                    <td class="px-3 py-1.5 font-mono text-gray-600">{param.type}</td>
                                                                </tr>
                                                            {/each}
                                                        </tbody>
                                                    </table>
                                            ...
                                            {#if api.returns}
                                                <div>
                                                    <h4 class="text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">Returns</h4>
                                                    <code class="text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded">{api.returns}</code>
                                                </div>
                                            {/if}

                                            <!-- Note -->
                                            {#if api.note}
                                                <div class="bg-amber-50 border-l-3 border-amber-400 px-3 py-2 text-xs text-amber-800 rounded-r">
                                            ...
                                            <div>
                                                <div class="flex items-center justify-between mb-1">
                                                    <h4 class="text-xs font-semibold text-gray-700 uppercase tracking-wider">Example</h4>
                                                    <button
                                                        onclick={(e) => { e.stopPropagation(); copyToClipboard(api.example, `${sIdx}-${aIdx}`); }}
                                                        class="flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded transition-colors {copiedSection === `${sIdx}-${aIdx}` ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
                                                    >
                                                ...
                                                <pre class="bg-gray-900 text-gray-100 p-3 rounded text-xs overflow-x-auto leading-relaxed"><code>{api.example}</code></pre>
                                            </div>
                                        </div>
                                    </div>
                                {/if}
